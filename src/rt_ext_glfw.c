...
RT_GLFW_SWAPCHAIN_PROCEDURES(RT_DEFINE_GLFW_DISPATCH)
...
#define RT_CLEAR_GLFW_PROCEDURE(return_type, name, parameters, arguments) rt_##name = NULL;
	RT_GLFW_SWAPCHAIN_PROCEDURES(RT_CLEAR_GLFW_PROCEDURE)
...
void rtLoadGlfwSwapchain(void) {
	...
	if (initialize && !initialize()) {
		...
		if (error == RT_SUCCESS) {
			fprintf(stderr, "rtLoadGlfwSwapchain backend initialization failed without recording an error\n");
		} else {
			fprintf(stderr, "%s\n", message && message[0] ? message : "rtLoadGlfwSwapchain backend initialization failed");
		}
		return;
	}
...
#define RT_RESOLVE_GLFW_PROCEDURE(return_type, name, parameters, arguments) \
	...
	if (!rt_##name) { \
		fprintf(stderr, "rtLoadGlfwSwapchain missing required procedure: %s\n", #name); \
		rtl_clear_glfw_extension(); \
		return; \
	}
	RT_GLFW_SWAPCHAIN_PROCEDURES(RT_RESOLVE_GLFW_PROCEDURE)
