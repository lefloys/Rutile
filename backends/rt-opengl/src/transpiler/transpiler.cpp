...
private:
	...
	std::uint32_t tessellationControlInvocations() const {
		...
		for (const rtsl::ir::EntryAttribute& attribute : entry.attributes) {
			...
			if (attribute.tokens.size() != 1) throw std::runtime_error("@invocations requires one numeric token");
			const std::string_view spelling = module.strings.get(attribute.tokens.front());
			std::uint32_t result{};
			const auto [end, error] = std::from_chars(spelling.data(), spelling.data() + spelling.size(), result);
			if (error != std::errc{} || end != spelling.data() + spelling.size() || result == 0)
				throw std::runtime_error("@invocations must contain a non-zero unsigned integer");
			return result;
		}
		throw std::runtime_error("tessellation-control entry point requires @invocations");
	}
